(ns research.phase
  "Phase 0->3 staged rollout -- the research analog of `cloud-
  ...
  submitting a real findings report is the ONE real-world research
  act this actor performs; it is always a human research-operator
  call. `research.governor`'s `:actuation/publish-findings-report`
  high-stakes gate enforces the same invariant independently -- two
  layers, not one, agree on this. `:risk/screen` is likewise never
  auto-eligible, at any phase -- the same posture every sibling's
  screening op has. Phase 3's `:auto` set here has only ONE member
  (`:study/intake`) -- this domain has no separate no-capital-risk
  'file' lifecycle distinct from the study record itself.")

(def read-ops  #{})
(def write-ops #{:study/intake :protocol/verify :risk/screen
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                        :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:study/intake :protocol/verify :risk/screen}              :auto #{}}
