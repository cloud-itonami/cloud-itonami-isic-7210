(ns research.operation
  "OperationActor -- one research operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (LabOps-
  LLM) is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Research Integrity Governor (:govern) and the
  ...
  One graph run = one research operation (intake -> advise -> govern
  ...
  decision to a human operator (research operator/PI). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/publish-findings-report` ALWAYS reaches this node when
  the governor is clean -- see `research.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [research.researchadvisor :as researchadvisor]
            [research.governor :as governor]
            [research.phase :as phase]
...
(defn build
  ...
  opts:
    :advisor      -- a `research.researchadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (researchadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LabOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (researchadvisor/-advise advisor store request)]
            {:proposal p :audit [(researchadvisor/trace request p)]})))

      ;; Research Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
